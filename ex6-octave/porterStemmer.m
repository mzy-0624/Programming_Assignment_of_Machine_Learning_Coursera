function stem = porterStemmer(inString)
    inString = lower(inString);     % Lower-case string
    global j;
    b = inString;
    k = length(b);
    k0 = 1;
    ...
    if k > 2
        ...
        x = step1ab(b, k, k0);          % disp(sprintf('Steps 1A and B yield: %s', x{1}));
        x = step1c(x{1}, x{2}, k0);     % disp(sprintf('Step 1C yields: %s', x{1}));
        x = step2(x{1}, x{2}, k0);      % disp(sprintf('Step 2 yields: %s', x{1}));
        x = step3(x{1}, x{2}, k0);      % disp(sprintf('Step 3 yields: %s', x{1}));
        x = step4(x{1}, x{2}, k0);      % disp(sprintf('Step 4 yields: %s', x{1}));
        x = step5(x{1}, x{2}, k0);      % disp(sprintf('Step 5 yields: %s', x{1}));
        stem = x{1};
    end
...
% setto(s) sets (j+1),...k to the characters in the string s, readjusting k accordingly.
...
% step2() maps double suffices to single ones. so -ization ( = -ize plus -ation)
