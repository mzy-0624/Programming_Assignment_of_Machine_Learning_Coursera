function [model] = svmTrain(X, Y, C, kernelFunction, tol, max_passes)
    % [model] = SVMTRAIN(X, Y, C, kernelFunction, tol, max_passes) trains an SVM classifier and returns trained model.
    ...
    L = 0;
    H = 0;
    % Pre-compute the Kernel Matrix since our dataset is small (in practice, optimized
    % SVM packages that handle large datasets gracefully will _not_ do this)
    % 
